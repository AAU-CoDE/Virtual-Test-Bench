...
function createCissVdsNetlist(fciss,Vds,cissPath,netlistFilename,userDef)
...
% netlist2code("RdsonTestBench.net",append(pwd,"\LTSpiceModelExtraction\Rdson"))
    ...
    netlist = [append(cissPath,netlistFilename)];
   ...
   % .subckt port parse 
    ...
    for fidFailCount = 1:5
        try
            fid = fopen(netlist, 'w+');
            fprintf(fid,cissTestBenchNetlist);
            fid = fclose(fid);
            break
        catch
            if fidFailCount == 5
